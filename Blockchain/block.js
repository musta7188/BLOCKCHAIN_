...
class Block {
  ...
  static mineBlock({lastBlock, data}){
    ...
    do { ///keep increasing the nonce and insert the data until the hash with the difficulty 
        ///requirement is met 
      nonce++;
      timestamp = Date.now();
    ...
    } while (fromHexToBinary(hash).substring(0, difficulty) !== '0'.repeat(difficulty))  //create the data has for the new block
